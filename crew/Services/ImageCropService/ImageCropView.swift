...
public struct ImageCropView: UIViewControllerRepresentable {
    ...
    public func makeUIViewController(context: Context) -> CropViewController {
        ...
        if croppingStyle == CropViewCroppingStyle.default {
            cropController.customAspectRatio = CGSize(width: 1040, height: 1682)

        } else if croppingStyle == CropViewCroppingStyle.circular {
